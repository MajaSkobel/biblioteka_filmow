...
def get_movies(library_titles):
    return get_items(library_titles, lambda item: type(item) == Movie)

# Function which sorts the series from the library
def get_series(library_titles):
    return get_items(library_titles, lambda item: type(item) == Series)
...
def top_titles(library_titles,number,content_type=None):
     if content_type == Series:
        condition=get_series
     elif content_type == Movie:
        condition=get_movies
     else:
        condition=lambda x: x
     library_titles_sorted=sorted(condition(library_titles), key = lambda item: item.views, reverse=True)
     for item in library_titles_sorted[:3]:
          print(item)

# Function completing the series for known series in the library
def complete_series(input_title,input_year,input_genre,season_number,episodes_number,library_titles):
    for i in range(episodes_number):
        new_episode = Series(title=input_title,year=input_year, genre=input_genre, season=season_number, episode=i+1)
        library_titles.append(new_episode)

# Classifying the library items
def classify(library):
    library_titles=[]
    for item in library:
        if item['type'] == 'movie':
...
if __name__ == "__main__":
    ...
    library_titles = classify(library)
